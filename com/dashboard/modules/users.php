...
class users extends App{
	...
	function newreg(){
		...
		if ($this->_p('submit')){
			...
			if(!$valid_email)
			{ 
				print json_encode(array('status'=>2,'msg'=>'* Invalid username'));
				exit;
			}
			...
			if($insertnewdata)
			{ 
				print json_encode(array('status'=>1,'msg'=>'New user has successfuly added'));
				exit;
			}
	...
	function registeredit()
	{
		...
		//check valid name
			...
			{
				$gros=array('msgu'=>'* Please enter the username','user'=>$username,'role'=>$role,'brand'=>$brandsubid,'startdate'=>$startdate,'enddate'=>$enddate);
				$this->assign('status',$gros);
				return $this->View->toString(TEMPLATE_DOMAIN_DASHBOARD .'apps/register-edit.html');
			}
			if ($startdate=='')
