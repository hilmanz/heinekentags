...
class news extends App{
	...
	function main(){
		...
		return $this->View->toString(TEMPLATE_DOMAIN_WEB .'apps/listnews.html');
		
	}
	function contentview_news()
	{
		global $LOCALE,$CONFIG;
	...
	//	pr($id);exit;
		...
		return $this->View->toString(TEMPLATE_DOMAIN_WEB .'apps/contentview_news.html');
	}
	...
	//	pr($_POST);exit;
		if ($this->_p('ajax')){
			$ajax =	$this->newsHelper->listnews($start);
		}
	//pr($ajax);
		if ($ajax){ 
