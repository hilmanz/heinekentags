...
class messageHelper {
	...
	function getMessage($start=0,$limit=10,$notification=true){
		...
		$result['total'] = 0;
		$result['totalnotif'] = 0;
		$result['pages']['nextpage'] = 0;
		$result['pages']['prevpage'] = 0;
		$uid = intval($this->apps->_p('uid'));
		if($uid!=0)if($this->uid!=$uid) return $result;
		...
		if($qData) {
			...
			foreach($qData as $key => $val){
				...
				$qData[$key]['message'] = str_replace(array('(notification)',"\\"),'',html_entity_decode($qData[$key]['message']));
			 
				if($val['messagetype']!='direct') $qData[$key]['message']  =  strtoupper(substr($qData[$key]['message'],1,1)).strtolower(substr($qData[$key]['message'],2));
	...
	function createMessage($recipientid=false,$message=false,$fromwho=1,$parentid=0,$multimessages=false,$attachment=false){
		...
		if($recipientid==4)$bugreport = 1;
				$this->logger->log(' is report bug :'.$bugreport);
		if($bugreport==1) {
				// $this->creatBugReport($recipientid,$message);
				$this->sendmessagetomail($recipientid,$this->apps->user->email,$message," [  BUG REPORT ] USER MESSAGE ",true,$attachment) ;
				return true;
		}
		
		$torecepientid = false;
		if($parentid!=0){
			$sql ="SELECT fromid,parentid FROM my_message WHERE id ={$parentid} AND fromid <> {$this->uid}  LIMIT 1";
		...
		if($this->apps->getLastInsertId()>0) {
			...
			$sql ="SELECT * FROM my_news_letter WHERE userid={$recipientid} AND type ='inbox' AND n_status = 0 LIMIT 1 ";
			$this->logger->log($sql);
	...
	function sendmessagetomail($recipientid=false,$from=false,$msg=false,$subject=false,$bugreport=false,$attachment=false){
			...
			$mailpin = $newsHelper->sendGlobalMail($to,$from,$msg,$subject,$bugreport,$attachment);
			$this->logger->log($to);
