...
class trackuserHelper {
	...
	function insertnewcareer(){
		global $CONFIG;
		//pr($_POST);exit;
		$name = strip_tags($this->apps->_p('name'));       
		$email = strip_tags($this->apps->_p('email')); 
		$telp = intval($this->apps->_p('telp')); 		
		$alamat = $_POST['alamat'];  
		$startdate =  date('Y-m-d H:i:s'); 
		//pr($startdate);exit;
	
		
		$sql = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.my_career (`name`, `email`,`address`,`telp`,`date`) 
							VALUES ('{$name}', '{$email}', '{$alamat}','{$telp}',NOW())";
		//	pr($sql);exit;	  	
		$res = $this->apps->query($sql);
	...
	function listnews($start=null,$limit=2)
	{
		global $CONFIG;
