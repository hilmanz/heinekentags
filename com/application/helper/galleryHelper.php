...
class galleryHelper {
	...
	function getFolder(){
		...
		$sql = "select *,
		(SELECT COUNT(*) AS total FROM  gallery WHERE  folder=mf.id_folder )AS total_gmbr
		from {$CONFIG['DATABASE'][0]['DATABASE']}.gallery gl
		LEFT JOIN 
		{$CONFIG['DATABASE'][0]['DATABASE']}.my_folder mf ON gl.folder = mf.id_folder
		GROUP BY mf.id_folder LIMIT 10
		";
		
		$res['result'] = $this->apps->fetch($sql,1);
		
		
		$sql = "select count(id_folder) as total from {$CONFIG['DATABASE'][0]['DATABASE']}.gallery gl
		LEFT JOIN 
		{$CONFIG['DATABASE'][0]['DATABASE']}.my_folder mf ON gl.folder = mf.id_folder
		
		";
		
		$res['total'] = $this->apps->fetch($sql)['total'];
